"""The official competition metrics from
...
class VQAEvaluator:
    ...
    def __init__(self, model_path='google/paligemma2-10b-mix-448'):
        ...
        # ).to('cuda:0')
        # Directly load from Hugging Face model hub
...
class AestheticEvaluator:
    def __init__(self, clip_model_path='ViT-L/14'):
        ...
        if clip_model_path is None:
            ...
            #self.clip_model_path = kagglehub.model_download(
    ...
    def score_differentiable(self, image):
        ...
        return self.predictor(image_features).mean()
...
def resize(img, size):
  ...
  if three_d:
    img = img.unsqueeze(0)
  resized = F.interpolate(img, size=(size, size), mode='bicubic',
                          align_corners=False, antialias=True)
  if three_d:
...
def prep_for_score(image):
  return ImageProcessor(
    image=image.resize((384, 384)), seed=random.randint(0, np.iinfo(np.int32).max)).apply().image.copy()


def prep_for_ocr(image):
  return (ImageProcessor(
     image.resize((384, 384)), seed=random.randint(0, np.iinfo(np.int32).max))
